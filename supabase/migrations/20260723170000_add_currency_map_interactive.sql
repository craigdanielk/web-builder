-- ═══════════════════════════════════════════════════════════════
-- CURRENCY-MAP/interactive — interactive currency map with plotted nodes
-- ═══════════════════════════════════════════════════════════════
-- Inserts a parameterized CURRENCY-MAP/interactive row into
-- section_archetypes with plotted currency nodes, interactive
-- hover/click detail, and a self-describing slot_schema.
--
-- Brand tokens consumed (existing):
--   {{brand.bg_primary}}           → e.g. "gray-50"
...
VALUES (
    'CURRENCY-MAP',
    'interactive',
    'Interactive world currency visualization with plotted exchange-rate nodes — hover for tooltip data, click for detail panel. All currency data, headline, and disclaimer driven by slot_schema; no hard-coded copy.',
    'framer-motion',
    TRUE,
    'section-templates/CURRENCY-MAP/interactive.tsx',
    $$"use client";
      ...
      "slots": [
        {
          ...
          "required": true,
          "default": "Global Currency Exchange Rates",
          "fill_prompt": "Headline for a currency exchange rate map section (2-8 words, e.g. \"Global Currency Rates\", \"Exchange Rate Overview\", \"World Currency Map\")",
          "description": "Primary headline — introduces the currency map section"
        },
        {
          "name": "subheadline",
          "source_path": "content.subheadline",
          "required": false,
          "default": "Real-time foreign exchange rates for major world currencies. Hover or tap any node for details.",
          "fill_prompt": "Supporting subheadline explaining the interactive currency map (1-2 sentences, 10-25 words)",
          "description": "Secondary text explaining the interactive map interaction"
        },
        {
          "name": "nodes",
          "source_path": "metrics.nodes",
          "required": true,
          "default": "[]",
          "fill_prompt": "Array of currency node objects with label, code, rate, change, direction (up/down/flat), x (0-100), and y (0-100) percentage position coordinates",
          "description": "Plotted currency exchange rate nodes — each with label, code, rate, change (with direction), and map position (x/y as 0-100 percentage)"
        },
        {
          "name": "disclaimer",
          "source_path": "content.disclaimer",
          "required": false,
          "default": "Rates shown are indicative market mid-rates. Actual transaction rates may vary.",
          "fill_prompt": "Disclaimer text about exchange rate accuracy (1 sentence, builds trust). Omit if not needed",
          "description": "Optional legal or trust-building disclaimer about exchange rates"
        }
      ]
    }'::jsonb
)
