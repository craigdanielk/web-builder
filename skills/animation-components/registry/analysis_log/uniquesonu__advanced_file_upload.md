# uniquesonu__advanced_file_upload

**Source:** `21st-dev-library/uniquesonu/advanced-file-upload.tsx`
**Component Type:** ui
**Framework:** none
**Animation Type:** exit
**Line Count:** 432

## Extracted Features

- **Triggers:** click
- **Motion Intents:** exit, drag
- **Interaction Intents:** engage, submit, upload
- **Supported Elements:** heading, card, button
- **Layout Contexts:** section
- **Section Archetypes:** none
- **Directionality:** vertical
- **Axis:** y

## Capabilities

- Stagger: false
- Interruptible: false
- Reversible: false
- Composable: false
- Looping: false
- Duration Range: 300ms - 800ms
- Easing: ease-out

## Performance and Risk

- GPU Accelerated: true
- Layout Shift Risk: low
- Scroll Coupling Risk: low
- Motion Stacking Risk: low
- Mobile Safe: true
- Accessibility Safe: true
- Reduced Motion Fallback: false
- Requires Layout Measurement: false

## Classification Rationale

Classified as **UI** component. Contains 8 hardcoded content strings.  Large component (432 lines) with structural layout.

## Uncertainty Notes

- No animation framework detected - classification relies on filename signals only.

## Fallback Assumptions

- Default duration: 300ms - 800ms (defaulted - no explicit duration found)
- Default easing: ease-out (defaulted)
